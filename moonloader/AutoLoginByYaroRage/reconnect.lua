...
-- Запись ника в реестр SA-MP, чтобы при переподключении клиент использовал правильный ник.
local function set_nick(only_registry)
    local s = AL.state
    local nick = s.my_nick or ""
    if nick == "" then return end
    AL.log("set_nick: применяю ник \"" .. tostring(nick) .. "\" только_реестр=" .. tostring(only_registry or false))
...
function M.handle_auth_limit_cef()
    ...
    lua_thread.create(function()
        ...
        -- Крайнее средство: эмуляция нажатия ESC, если окно все еще не закрылось
        if not s.is_spawned and not s.is_reconnecting and not s.login_submitted then
...
function M.do_fast_reconnect(seconds)
    ...
    lua_thread.create(function()
        ...
        -- Реконнект завершён: снимаем флаг "в процессе",
