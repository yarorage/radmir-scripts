...
-- Проверка опоры: есть ли геометрия вниз от педа (если нет - пед падает/провалился)
...
local function is_path_clear(px, py, pz, heading, distance)
    ...
    -- Проверка обрыва: если в точке впереди ниже на глубину AFK_CLIFF_DEPTH нет земли - идти нельзя
...
local function hold_run(duration, backwards, heading_deg)
    ...
    while elapsed < duration do
        ...
        if move_counter >= 25 then
            ...
            -- Защита от проваливания: пед резко ушёл вниз и под ним пусто - останавливаем движение
...
-- Детектор проваливания педа под карту/текстуры.
-- Если пед долго падает без опоры (под ним нет земли) - выполняем реконнект,
-- чтобы не остаться под картой. После входа mafk_active сохраняется и движение возобновится.
function M.mafk_under_map_watch()
    ...
    while true do
        ...
        if fall_dist > (s.AFK_UNDER_MAP_FALL_DIST or 25.0) and fall_time >= 2.0 then
            AL.chat_msg("{FF6600}[Anti-AFK]{FFFFFF} Провал под карту, реконнект...")
            AL.log("Anti-AFK: провал под карту, выполняю быстрый реконнект")
            pcall(function()
