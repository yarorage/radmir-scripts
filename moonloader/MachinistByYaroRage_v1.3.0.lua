...
-- v1.2.9: автовыбор «Жалоба на игрока» в CEF-окне типа запроса (/report).
-- Когда клиенту приходит addDialogInQueue «Выберите тип запроса», Machinist
-- автоматически выбирает пункт «Жалоба на игрока» (index=0) и отправляет
-- TX OnDialogResponse id=215, даже если игрок отошёл от клавиатуры.
-- Формат TX подтверждён реальными пакетами 15.09.2026 (разбор CefPacketAnalyzer:
-- Sonya_Tekilla 14 запросов OnDialogResponse): параметры [0][1][index][строка],
-- строка = '<p style="color: #">1. Жалоба на игрока</p>' (CP1251, 43 байта),
-- номер пункта «1.» подставляет клиент.
local reportAnswer = {
    pending = false,
    at = 0,
    last = 0,
...
-- Постановка автовыбора в очередь (кулдаун 40 с, чтобы не спамить, если
-- сервер периодически шлёт тот же диалог).
local function scheduleReportAnswer(index, text)
    if reportAnswer.pending then return end
    local now = wallClockMs()
    if now - reportAnswer.last < 40000 then return end
    reportAnswer.pending = true
    reportAnswer.index = index or 0
    reportAnswer.text = text or ""
    reportAnswer.at = now + 2200   -- дать CEF-окну прогрузиться
end

-- Отправка отложенного автовыбора (вызывается из главного цикла).
local function trySendReportAnswer()
    if not reportAnswer.pending then return end
    if wallClockMs() < reportAnswer.at then return end
    reportAnswer.pending = false
    if not reportAnswer.text or #reportAnswer.text == 0 then return end
    local ok, err = pcall(cef.send_dialog_response, reportAnswer.index, reportAnswer.text)
    if ok then
        reportAnswer.last = wallClockMs()
    else
        print("[MachinistByYaroRage] ошибка отправки автовыбора: " .. tostring(err))
    end
end
...
function onReceivePacket(id, bs)
    ...
    if not st.dbg_no_thread and optEnabled.v then
        ...
        if rxtMs - (drive._lastTickMs or 0) >= 200 then
            -- v1.2.4: резервный тик (свёрнутое окно) — спидхак не применяем.
    ...
    -- v1.2.9: автовыбор «Жалоба на игрока» (окно /report). Текст «Выберите тип
    -- запроса» не содержит "Machinist", поэтому хук ставим ДО раннего return
    -- ниже. Детект по подстрокам RX-строки (CP1251): addDialogInQueue + заголовок
    -- + пункт списка.
    if optEnabled.v then
        local repOk, rep = pcall(cef.parse_report_dialog, txt)
        if repOk and rep then scheduleReportAnswer(rep.index, rep.text) end
    end
...
local function driveTick()
    ...
    if stopCmd then
        stopping = true
        stopHard = true
        -- v1.2.5: флаг остановки по таймеру «Остановитесь на станции»:
        -- по завершении этого таймера сразу отправляемся, не дожидаясь
        -- исчезновения прочих таймеров (штраф за низкую скорость и т.п.).
    ...
    -- v1.2.5: фриз действует ТОЛЬКО пока активен таймер «Остановитесь на
    -- станции» и максимум stopFreezeSec секунд; прочие таймеры (штраф за
    -- низкую скорость и т.п.) фриз не продлевают — поезд должен тронуться.
    ...
    if speed < 3 and (stopCmd or stayCmd or atStopPoint) and not crawling then
        ...
        if canGo then
            ...
            releaseBrake()
            -- v1.2.5: жёсткий фриз только по командам стоянки (таймер
            -- «Остановитесь на станции» / «Стоять»); штрафные таймеры и
            -- прочие посторонние не замораживают поезд — он может уехать.
...
function driveThread()
    ...
    while optEnabled.v do
        wait(0)
        -- v1.2.4: кадровый тик — всегда обычный (не резервный), флаг сброшен.
        drive._reserveTick = false
        driveTick()
    end
    drive.tickThread = nil
