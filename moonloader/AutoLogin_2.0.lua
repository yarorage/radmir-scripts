...
local function trigger_reconnect()
    ...
    lua_thread.create(function()
        ...
        if hwnd ~= nil and user32.IsIconic(hwnd) == 0 and user32.GetForegroundWindow() == hwnd then
            ...
            log("Реконнект: F11 (один раз). Ждём Authorization, пароль не вводим вслепую.")
            wait(800)
...
start_reconnect_watch = function()
    ...
    log("Watchdog реконнекта: 35 сек (ждём Загрузка / очередь / подключение)")
    lua_thread.create(function()
        while reconnect_watch_active and os.clock() < reconnect_watch_until do
            ...
            if saw_loading_after_rec or saw_queue_after_rec or is_spawned or login_submitted or connected then
                reconnect_watch_active = false
                log("Watchdog: ок (загрузка/очередь/спавн/вход/подключен) — F11 не нужен")
                return
            end
        end
        if reconnect_watch_active
            ...
            if connected_now then
                reconnect_watch_active = false
                log("Watchdog: подключение активно (" .. tostring(connected_now) .. ") — ждём логин, F11 не шлём")
                return
            end
            reconnect_watch_active = false
            log("Watchdog: 35с без загрузки и без очереди — принудительный F11 (spawned=" .. tostring(is_spawned) .. ", login_submitted=" .. tostring(login_submitted) .. ", pending=" .. tostring(pending_autologin) .. ")")
            is_spawned = false
            is_logging_in = false
            lua_thread.create(function()
...
schedule_autologin_after_reconnect = function()
    ...
    is_spawned = false
    is_logging_in = false
    log("schedule: ждём пакет Authorization[auth] (без слепого ввода)")
    lua_thread.create(function()
        for i = 1, 40 do
...
function sampevents.onServerJoin()
    ...
    pending_autologin = true
    last_login_attempt = 0
    log("onServerJoin — ждём CEF Authorization / автовход")
...
local function timer_render_thread()
    ...
    while true do
        ...
        if is_password_form_visible() then
            ...
            renderFontDrawText(r_font, "ЛКМ по полю — активировать ввод", form_x + pad + 56, hint_y + 26, 0xFF7A7A8A)
            renderFontDrawText(r_font, "Ctrl+V — вставить    Ctrl+C — копировать", form_x + pad, hint_y + 48, 0xFF8888B8)
