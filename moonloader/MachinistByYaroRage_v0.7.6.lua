    ...
    while optEnabled.v do
        ...
        if not inTrain then
            ...
            if nearStation then
                ...
                if st.overspeed and st.speed_hi and st.speed_hi > 0 then
                    useTarget = st.speed_hi + (st.overspeed_extra or 8)
                end
                -- v0.7.6: если сервер запустил таймер штрафа (overspeed_fine) —
                -- начинаем тормозить заранее (за overspeed_guard секунд до конца
                -- таймера), чтобы к штрафу скорость уже была в вилке.
                local brakeNow = false
                if st.overspeed and st.overspeed_fine and st.overspeed_timer > 0
                   and st.speed_hi and st.speed_hi > 0 then
                    local tLeft = st.overspeed_timer - os.time()
                    local vMs = math.max(0, speed / 3.6)
                    local hiMs = st.speed_hi / 3.6
                    local tBrake = (vMs - hiMs) / 2.8
                    if tLeft <= tBrake + (st.overspeed_guard or 1) and tLeft > 0.2 then
                        brakeNow = true
                    end
                end
                if brakeNow then
...
function main()
    ...
    print(string.format("[MachinistByYaroRage] v0.7.6 флаги: no_thread=%d no_events=%d no_gui=%d no_chat=%d tg_poll=%d dbg_log=%d force_cab=%d",
