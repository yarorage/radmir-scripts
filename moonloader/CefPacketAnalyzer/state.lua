...
M.state = {
    ...
    -- Канал отправки агрегатов. true (по умолчанию) = неблокирующий async движка:
    -- запрос идёт в фоновом потоке (WinHTTP), игровой поток не блокируется, фризов
    -- нет, внешние процессы не запускаются; если движок не даёт async.httpPost —
    -- автоматический фолбэк на файл в pending/ (отправку делает ПК-сборщик).
    -- false = синхронный (резерв для ручной диагностики, может слегка заблокировать поток).
