...
-- v0.9.4: кэш кабины на 200 мс. Нативный game-вызов isCharInAnyTrain из
-- wait(0)-потока в обычной машине вызывался каждый кадр (60 раз/с) — по
...
-- до ~1 кадра/с). Теперь нативный опрос не чаще 5 раз/с; задержка
-- распознавания входа/выхода из поезда — до 200 мс (незаметно). При
-- свёрнутом окне кадров нет, кэш протухает, и опрос идёт из резервного
-- ev.onSendVehicleSync вместе с сетевыми пакетами — как и раньше.
    ...
    drive._gasPressed = false
        ...
        if not inTrain then
            ...
            if stationAhead then
                ...
                -- v0.9.3: команду «Остановитесь на станции» признаём даже если
                -- сервер не прислал число секунд (info_timer_sec равен 0 или
                -- отсутствует) — иначе «не ловился» триггер окончательной
                -- остановки и состав без stopCmd долго болтался у станции.
                ...
                -- v0.9.3: как и в DRIVE-ветке — экстренный сброс раньше и
                ...
                elseif speed > effLim + 1 then
                    ...
                    -- v0.9.3: ДОВОД ДО ТРИГГЕРА. Если команда остановки так и
                    -- не пришла, а скорость уже упала почти до нуля до того,
                    -- как пройдены +20 м за маркер (atTrigger=false) — старый
                    -- код замирал навсегда: allowed стремился к нулю, триггер
                    -- не защитывался, станция «не ловилась». Ползём вперёд на
                    -- 8 км/ч (setTrainSpeed), пока нос не войдёт в точку
                    -- останова, и уже там встаём на стоянку.
                    if not stopCmd and not atTrigger and speed < 1
                        ...
                        -- зовём не чаще раза в 250 мс, иначе «подползание» на
                ...
                -- v0.9.3: ПАУЗА ПОСЛЕ ШТРАФА. Как только таймер «Снизьте скорость»
                -- истёк, старый код немедленно снова разгонялся выше вилки, а
                -- сервер сразу вешал новый таймер — штрафы шли пачками. Теперь
                -- после окончания штрафа ещё несколько секунд держим вилку.
                ...
                -- max замедления) и в последние минимум 2 секунды таймера —
